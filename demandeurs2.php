 ...
 */
    ...
    class requestusersPage extends Page
    {
        protected function DoBeforeCreate()
        {
            $this->dataset = new TableDataset(
                ...
                '`requestusers`');
            $field = new IntegerField('RequestUserId', null, null, true);
        ...
        {
            return array(
                new FilterColumn($this->dataset, 'RequestUserId', 'RequestUserId', 'Id'),
                new FilterColumn($this->dataset, 'UserId', 'UserId_NamePlusUsername', 'Demandeur')
            );
        }
        ...
        {
            $main_editor = new TextEdit('requestuserid_edit');
        ...
        {
            //
            // View column for RequestUserId field
            //
            $column = new StringTransformViewColumn('RequestUserId', 'RequestUserId', 'Id', $this->dataset);
            ...
            //
            $column = new TextViewColumn('UserId', 'UserId_NamePlusUsername', 'Demandeur', $this->dataset);
            $column->SetOrderable(true);
            $column->setMinimalVisibility(ColumnVisibility::PHONE);
            $column->SetDescription('');
            $column->SetFixedWidth(null);
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            $editColumn = new DynamicLookupEditColumn('Demandeur', 'UserId', 'UserId_NamePlusUsername', 'edit_UserId_NamePlusUsername_search', $editor, $this->dataset, $lookupDataset, 'UserId', 'NamePlusUsername', '');
        ...
        protected function AddInsertColumns(Grid $grid)
        {
            ...
            $lookupDataset->AddField($field, false);
            $lookupDataset->setOrderByField('NamePlusUsername', GetOrderTypeAsSQL(otAscending));
            $editColumn = new DynamicLookupEditColumn('Demandeur', 'UserId', 'UserId_NamePlusUsername', 'insert_UserId_NamePlusUsername_search', $editor, $this->dataset, $lookupDataset, 'UserId', 'NamePlusUsername', '');
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $this->RenderText($editColumn->GetCaption())));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
        ...
        protected function CreateGrid()
        {
            ...
            $this->setDescription('Demandeurs d\'examen de radiologie');
            $this->setDetailedDescription('Gestion de la liste des demandeurs d\'examen de radiologie sur les patients (Généralement des médecins, inférmiers,..etc)');
    ...
    try
    {
        $Page = new requestusersPage("requestusers", "demandeurs2.php", GetCurrentUserPermissionSetForDataSource("requestusers"), 'UTF-8');
        $Page->SetTitle('Demandeurs');
        $Page->SetMenuLabel('Demandeurs');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("requestusers"));
